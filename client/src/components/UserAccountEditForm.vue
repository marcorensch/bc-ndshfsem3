      ...
      <div class="row">
        ...
        <div class="col-md-6">
          ...
          <span v-if="v$.user.username.$error"
                :class="`${v$.user.username.$error ? 'error-message' : ''}`">{{
        ...
        <div class="col-md-6">
          ...
          <span v-if="v$.user.email.$error"
                :class="`${v$.user.email.$error ? 'error-message' : ''}`">{{ v$.user.email.email.$message }}</span>
        </div>
      </div>

      <div class="row mt-3">
        <div class="col-md-12">
          Change your Password or leave empty if you don't want to change it
        </div>
        <div class="col-md-6">
          <label for="new-password">New password</label>
          <input type="password" class="form-control" id="new-password" v-model="password" autocomplete="off">
          <span v-if="v$.password.$error"
                :class="`${v$.password.$error ? 'error-message' : ''}`">{{"Min. " + v$.password.minLength.$params.min + " characters" }}</span>
        </div>
        <div v-if="password.length || passwordConfirmation.length" class="col-md-6">
          ...
          <span v-if="v$.passwordConfirmation.$error" :class="`${v$.passwordConfirmation.$error ? 'error-message' : ''}`">{{"Not same password" }}</span>
        </div>
      </div>
