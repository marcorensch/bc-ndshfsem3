...
class QuestionHelper {
    ...
    async _updateVote(id, voting) {
        const sql = `UPDATE question_votes
                     SET voting=?
                     WHERE id = ?`;
        try {
    ...
    async _createVote(questionId, userId, voting) {
        const sql = `INSERT INTO question_votes (question_id, user_id, voting)
                     VALUES (?, ?, ?)`;
        try {
    ...
    async storeTags(questionId, tags) {
        ...
        let sql = `INSERT IGNORE INTO tags (title, alias)
                   VALUES ${values}`;
        try {
    ...
    async _linkTagsToQuestion(createdQuestionId, tags) {
        ...
        console.log(values);
        let sql = `INSERT IGNORE INTO question_tags (tag_id, question_id)
                   VALUES ${values}`;
        try {
            await this.databaseConnector.query(sql);
        } catch (error) {
